...
//     // Accept ANY uploaded asset (screenshot, PDF, PPTX — anything not
//     // deleted) OR a Figma URL as satisfying the "show us something"
//     // requirement. Previously this only counted SUBMISSION_PDF /
//     // SUBMISSION_PPTX assets, which rejected submissions that only had
//     // screenshots (the actual UI-Verse flow) even with no Figma URL set.
//     const hasAnyContent = activeAssets.length > 0;

//     if (!submission.figmaUrl && !hasAnyContent) {
//       throw new ConflictException(
//         'At least one file (screenshot, PDF, or PPTX) or a Figma URL is required',
//       );
//     }
...
export class SubmissionsService {
  ...
  async submit(submissionId: string, userId: string) {
    ...
    // DEV/TESTING RELAXATION: originally blocked re-submitting anything
    // already SUBMITTED, which made repeated manual end-to-end testing
    // impossible without a new user every time. Now only a genuinely
    // LOCKED submission (post-lock, e.g. after judging starts) is blocked.
    // Revisit before any real event goes live — a real participant
    // probably shouldn't be able to silently resubmit indefinitely.
